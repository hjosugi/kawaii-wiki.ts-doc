...
> ソース: `apps/server/README.ja.md`

> 実装状況: source `main`ではPostgreSQLとMySQLを選択可能です。下記のリポジトリREADMEには移行途中の説明が残っているため、検証済みの現在動作は [[docs/database-and-search|データベースと検索]] を参照してください。
